...
open class CodeGenerator(
    ...
    private fun writeExtension(field: File.Field) {
        when (field) {
            is File.Field.Numbered -> {
                val localType = when (field.type) {
                    File.Field.Type.MESSAGE -> ((field as File.Field.Numbered.Standard).localTypeName ?: field.kotlinLocalTypeName)?.substringAfter(".")
                    else -> field.type.standardTypeName
                }
                line(
                    "val pbandk.wkt${field.extendee!!.substringAfterLast(".google.protobuf")}.${field.name}: ${localType}? get() ="
                )
                line("  getExtension(${file.packageName}.${field.name})")
                line()
                line("val ${field.name} = pbandk.FieldDescriptor(").indented {
                    line("name = \"${field.name}\",")
                    line("number = ${field.number},")
                    line("type = ${field.fieldDescriptorType(false)},")
                    line("value = pbandk.wkt${field.extendee!!.substringAfterLast(".google.protobuf")}::${field.name},")
                    line("messageDescriptor = pbandk.wkt${field.extendee!!.substringAfterLast(".google.protobuf")}.Companion::descriptor")
                }
                line(")")
            }
        }
    ...
    protected fun writeMessageType(
        ...
        val parentPrefix = parentType?.let { "${it}." }.orEmpty()
        val typeName = "${parentPrefix}${type.kotlinTypeName}"
        val messageInterface = if (type.extensionRange.isNotEmpty()) "pbandk.ExtendableMessage" else "pbandk.Message"

        var extends = messageInterface
        ...
        line().line("data class ${type.kotlinTypeName}(").indented {
            ...
            if (messageInterface == "pbandk.ExtendableMessage") {
                lineEnd(",")
                line("override val extensionFields: MutableMap<Int, kotlin.Any> = mutableMapOf()")
            } else {
                lineEnd()
            }
            ...
            // Companion object
            line("@kotlin.native.concurrent.ThreadLocal")
    ...
    protected fun writeMessageDescriptor(type: File.Type.Message, fullTypeName: String) {
        ...
        line("override val descriptor: pbandk.MessageDescriptor<$fullTypeName> by lazy {").indented {
            ...
            if (needToChunk) {
                line("val fieldsList = ArrayList<pbandk.FieldDescriptor<$fullTypeName, *>>(${chunkSize})")
                allFields.chunked(chunkSize).forEachIndexed { index, chunk ->
                    line("addFields${index}(fieldsList)")
                }
            } else {
                addFields(allFields, fullTypeName, line("val fieldsList = ArrayList<pbandk.FieldDescriptor<$fullTypeName, *>>(${allFields.size}).apply {"))
            }
    ...
    private fun generateUnknownFields(unknownFields: Map<Int, UnknownField>) {
        line("unknownFields = mapOf(").indented {
            unknownFields.entries.forEach {
                line("${it.key} to pbandk.UnknownField(").indented {
                    line("fieldNum = ${it.value.fieldNum},")
                    when (it.value.value) {
                        is UnknownField.Value.LengthDelimited -> line("value = pbandk.UnknownField.Value.LengthDelimited(bytes=pbandk.ByteArr(byteArrayOf(${(it.value.value as UnknownField.Value.LengthDelimited).bytes.array.joinToString()})))")
                        is UnknownField.Value.Composite -> line("value = pbandk.UnknownField.Value.Composite(values=listOf(${
                            getValue(
                                it.value.value
                            )
                        }))")
                        else -> throw UnsupportedOperationException("Unsupported unknown field type: '${it.value.value}'")
                    }
                }.line(")")
            }
        }.lineBegin(")")
    ...
    private fun getValue(it: UnknownField.Value) =
        (it as UnknownField.Value.Composite).values.joinToString {
            "pbandk.UnknownField.Value.LengthDelimited(bytes=pbandk.ByteArr(byteArrayOf(${(it as UnknownField.Value.LengthDelimited).bytes.array.joinToString()})))"
        }
